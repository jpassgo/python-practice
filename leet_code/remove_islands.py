...
# 		depth first search find all connected 1s and add them to our islandsToNotModify
				islandsToNotModify = depthFirstSearch(islandsToNotModify, x+1, y, matrix)
		y += 1
...
# 		step into the interior and check if that value is 1
			if matrix[x][y+1] == 1:
# 		depth first search find all connected 1s and add them to our islandsToNotModify
				islandsToNotModify = depthFirstSearch(islandsToNotModify, x, y+1, matrix)
		x += 1
...
def depthFirstSearch(islandsToNotModify, x, y, matrix):
	...
	if x > 1 and matrix[x-1][y] == 1 and not islandExistsInMap(islandsToNotModify, x-1, y):		
		return depthFirstSearch(islandsToNotModify, x-1, y, matrix)
	elif x < len(matrix) - 1 and matrix[x+1][y] == 1 and not islandExistsInMap(islandsToNotModify, x+1, y):
		return depthFirstSearch(islandsToNotModify, x+1, y, matrix)
	elif y > 1 and matrix[x][y-1] == 1 and not islandExistsInMap(islandsToNotModify, x, y-1):
		return depthFirstSearch(islandsToNotModify, x, y-1, matrix)
	elif y < len(matrix[x]) - 1 and matrix[x][y+1] == 1 and not islandExistsInMap(islandsToNotModify, x, y+1):
		return depthFirstSearch(islandsToNotModify, x, y+1, matrix)
	else:
